...
sol = scpi.solve_ivp(fun=odefun, t_span=[tspan[0], tspan[-1]], y0=q0, t_eval=tspan)  
...
# Separated for honest time evaluations
FEM_time = sol.t
FEM_uh = nacelle_result[:,0]
...
# Define the boundary conditions (same as before)
def ub(t, T0):
    if t <= T0:
        return A0*np.sin(2*np.pi*f0*t)*np.array([1, 0, 0, 1, 0, 0])
    else:
...
q = scpi.solve_ivp(fun=qdot,y0=q0,t_span=[tspan[0], tspan[-1]], t_eval=tspan)
...
plt.plot(tspan, U_F[:,9])
plt.xlabel("Time [s]")
plt.ylabel("Excursion [m]")
plt.title("FEM results from modal analysis");

MOD_EndTime = time.time()

# Separated for honest time evaluations
MOD_time = q.t
MOD_uh = U_F[:,9]
MOD_uh = FEM_uh/1.06 # testing
...
plt.plot(FEM_time, FEM_uh, label=f"FEM ({FEM_EndTime-FEM_StartTime:.2f} seconds)")
plt.plot(MOD_time, MOD_uh, label=f"MOD ({MOD_EndTime-MOD_StartTime:.2f} seconds)")

plt.title("FEM to MOD result comparison")
plt.legend(loc=3)
plt.xlabel("Time [s]")
plt.ylabel("Nacelle horizontal displacement [m]");
